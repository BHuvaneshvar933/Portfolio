...
const ParticleBackground = () => {
  const isBatteryPoweredRef = useRef(false);
  ...
  useEffect(() => {
    ...
    async function initBatteryStatus() {
      ...
      try {
        ...
        const update = () => {
          isBatteryPoweredRef.current = battery.charging === false;
        };
        ...
        // Ignore if unsupported/blocked
      }
    }

    let cleanupBatteryListener;
    initBatteryStatus().then((cleanup) => {
      cleanupBatteryListener = cleanup;
    });
    ...
    function animate(time) {
      ...
      const frameDelay = isBatteryPoweredRef.current ? 32 : 16; // 30fps vs 60fps
